...
export class ProgressStore {
  ...
  private static pageKey = ProgressStore.slugFromPathname(
  ...
  public static subscribePageDone(
    path: string,
  ...
  private static store(): void {
    ...
    try {
      localStorage.setItem(
        ProgressStore.key,
        JSON.stringify(ProgressStore.state),
      );
    } catch {
  ...
  /** Work out if all of this page’s active lists are complete. */
  private static get isPageDone(): boolean {
    if (ProgressStore.activeLists.size === 0) return false;
    return Array.from(ProgressStore.activeLists).every((listKey) =>
      ProgressStore.isListDone(ProgressStore.pageState.lists[listKey]),
    );
  }
