...
Linux utilitza repositoris ("repos") per gestionar el programari de forma segura. Generalment no es descarreguen fitxers `.exe` del web.

## Debian / Ubuntu (`apt`)

Utilitza paquets `.deb` (veure [[/ca/modules/module-1/1-overview|Famílies de Distribucions]]).
...
<Aside type="note">
  En sistemes RHEL/CentOS 7 antics, veuràs `yum`. En RHEL 8/9, `yum` és només un
  enllaç simbòlic a `dnf`.
</Aside>

## Exercicis Pràctics

1. **Actualitzar Primer**:
    - Executa el comandament d'actualització apropiat per al teu sistema (`apt update` o `dnf check-update`).
2. **Instal·lar Eines**:
