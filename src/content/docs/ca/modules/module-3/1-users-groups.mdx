...
L'autenticació en Linux gira al voltant de tres fitxers de text pla a `/etc`.
...
(Veure [[/ca/modules/module-2/4-permissions|Gestió de Permisos]] per a detalls sobre els modes 644/600)
...
# Establir o canviar contrasenya
sudo passwd anna
...
Executa un sol comandament amb privilegis de root. Segur i registrat.
...
<Aside type="caution">
  Evita iniciar sessió com a root directament per a tasques diàries. Utilitza `sudo` en
  el seu lloc per prevenir danys accidentals al sistema.
</Aside>
...
2.  **Treball en Grup**:
    ...
    - Verifica amb el comandament `groups` (p. ej., `groups intern`).
3.  **Neteja**:
