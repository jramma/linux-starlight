...
```mermaid
graph LR
    Raiz["/ (Directorio Raíz)"]
    
    Raiz --> bin["/bin & /sbin (Binarios)"]
    Raiz --> boot["/boot (Arranque)"]
    Raiz --> dev["/dev (Dispositivos)"]
    Raiz --> etc["/etc (Configuración)"]
    Raiz --> home["/home (Usuarios)"]
    Raiz --> rootDir["/root (Superusuario)"]
    Raiz --> tmp["/tmp (Temporales)"]
    Raiz --> usr["/usr (Programas)"]
    Raiz --> var["/var (Datos Variables)"]
```
