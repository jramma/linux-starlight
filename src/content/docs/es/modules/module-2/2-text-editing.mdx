...
- `:w` -> Guardar.
- `:q` -> Salir (si no has hecho cambios).
- `:wq` -> Guardar y Salir.
- `:q!` -> **FORZAR SALIDA SIN GUARDAR**. Esto destruye tus cambios y te salva la vida si has modificado accidentalmente un archivo crítico.
- `dd` -> Borrar la línea actual entera.
- `u` -> Deshacer tu último error (_Undo_).
